...
You can create your own custom links by extending the [BaseLink](https://github.com/archimedes-projects/archimedes-js/blob/0f9c51ac387e9bdb59e5cae911ef04c9b3b55acd/packages/arch/src/runner/links/base-link.ts) class. For instance imagine if we wanted to create a link that starts a spinner when executing an use case:
...
And then register it in the runner:

```ts
Runner.createChain([new LoadingLink(new Loading()), new ExecutorLink()])
```
